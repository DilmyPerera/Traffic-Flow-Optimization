...
for i in range(n):
    ...
    model += g[i] >= gmin  # ensure green time is not less than minimum green time
    model += g[i] <= gmax  # ensure green time is not more than maximum green time
...
if model.status != 1:
    ...
    # Plot green times with different colors
